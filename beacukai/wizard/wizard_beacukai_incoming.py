...
class wizard_product_incoming(models.TransientModel):
	_name = "wizard.product.incoming"
	_description = "Incoming Products"
	...
	def action_open_window(self):
		...
		result = data_obj._get_id('beacukai', 'view_beacukai_document_line_in_tree')
		domain = [('shipment_type','=','in'),('state','=','validated')]
		...
		return {
			'name': _('Laporan Penerimaan Barang'),
			'view_type': 'form',
			'view_mode': 'tree',
			'res_model': res_model,
			'view_id':[view_id],
	...
	def export_excel(self):
		self.ensure_one()
		datas = {
			'model': 'wizard.product.incoming',
			'from_date' : self.from_date,
			'to_date' : self.to_date,
			'product_type':self.product_type,
			'shipment_type':self.shipment_type,
			}
		report_name = 'beacukai.incoming_doc_xlsx'
